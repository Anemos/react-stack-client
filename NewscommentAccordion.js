...
const NewscommentAccordion = props => {
 const { movies } = props;
 const style = {
    color: "red",
    fontSize: 10,
 ...
 return (
    <div style={{ width: "100%", margin: "0 auto" }}>
     <h3 style={{textAlign: "center", margin: "10px auto auto"}}> 댓글 뉴스 TOP5 </h3>
...
<Image src={item.news_photo_src} className="thumbnail" thumbnail />
    ...
    <Col xs={10} md={10} lg={10}>
      <Badge pill variant="primary">{item.simplecnt}</Badge>{' '}{item.news_title}
    </Col>
  </Row>
</Container>
               </Accordion.Toggle>
      ...
      <div>
        <Badge pill variant="dark">총반응수: {item.sum_count}</Badge>{' '}
...
export default NewscommentAccordion;